...
# MagPython/libffi-msvc-include/ffi.h to a new release on the 3.x line.
#
...
#   2. Inline regen of MagPython/libffi-msvc-include/ffi.h from
#      upstream's include/ffi.h.in template. autoconf normally does
#      these @VAR@ substitutions at ./configure time; upstream
#      doesn't ship a Windows configure equivalent for x86_win32, so
#      we do it here. The committed ffi.h is what LibFFI.vcxproj's
#      <ClInclude> picks up at build time.
#
#   Also prints the current MagPython/libffi-msvc-include/fficonfig.h
...
mkdir -p "$SCRIPT_DIR/libffi-msvc-include"
sed \
    ...
    "$template" > "$SCRIPT_DIR/libffi-msvc-include/ffi.h"

# Sanity-check: any @VAR@ tokens still in the output mean we missed
# one in the substitution pass above.
if grep -nE '@[A-Z_][A-Z0-9_]*@' "$SCRIPT_DIR/libffi-msvc-include/ffi.h" >/dev/null 2>&1; then
    echo "ERROR: unsubstituted @VAR@ tokens remain in regenerated ffi.h:" >&2
    grep -nE '@[A-Z_][A-Z0-9_]*@' "$SCRIPT_DIR/libffi-msvc-include/ffi.h" >&2
    exit 75
fi

echo ""
echo "Regenerated MagPython/libffi-msvc-include/ffi.h from upstream template."
...
# changed since the previous bump, the project-local fficonfig.h may
# need to be regenerated by hand on a Linux/macOS host.
template="$SRC/fficonfig.h.in"
if [ -f "$template" ]; then
    echo ""
    echo "If MagPython/libffi-msvc-include/fficonfig.h needs updating,"
    echo "regenerate it on a Linux/macOS host:"
    echo "  cd $SRC"
    echo "  ./configure --host=i686-w64-mingw32 --enable-static"
    echo "  cp fficonfig.h $SCRIPT_DIR/libffi-msvc-include/fficonfig.h"
fi
