# Download and unpack zlib. Mirrors download-libmpdec.ps1's
...
# (zlib-version, zlib-sha256) so common.props, build-common.sh
# (Linux/macOS), and this script all share one source of truth.
# madler/zlib does not publish per-tarball .sha256 sidecars on its
...
$ExpectedSha256 = (Get-Content (Join-Path $ScriptDir 'zlib-sha256') -Raw).Trim().ToLower()
$Tarball = "zlib-$Version.tar.gz"
$BaseUrl = "https://github.com/madler/zlib/releases/download/v$Version"
$ExtractDir = Join-Path $ScriptDir "zlib\zlib-$Version"

if (Test-Path $ExtractDir) { exit 0 }

Push-Location $ScriptDir
try {
    if (!(Test-Path zlib)) { New-Item -ItemType Directory -Path zlib | Out-Null }
    Push-Location zlib
    try {
        ...
        if ($ExpectedSha256 -ne $actual) {
            ...
            throw "SHA-256 mismatch for ${Tarball}: expected $ExpectedSha256, got $actual (pinned in MagPython/zlib-sha256 -- regenerate via MagPython/update-zlib.sh before changing)"
        }
