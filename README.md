...
1. **`LibFFI.vcxproj`** — builds `libffi.lib` (static) from the
   ...
   `ml64.exe`). Both use project-local pregenerated MSVC headers
   under `MagPython/libffi-msvc-include/{x86,x64}/` (upstream only
   ships pregenerated MSVC headers for aarch64; the x86_win32 and
   x86_win64 variants are maintained here, regenerated by
   `update-libffi.sh` on each version bump).
...
(under `msvc_build/aarch64/`); it has never published an x86 or x64
Windows variant. The `ffi.h` and `fficonfig.h` we need therefore live
under `MagPython/libffi-msvc-include/{x86,x64}/`, outside the
build-time-downloaded tree (which gets wiped on each tarball extract).
`LibFFI.vcxproj` picks the right subdir from `$(Platform)`.
...
- **`ffi.h`** is regenerated on every `update-libffi.sh` run from
  ...
  `@TARGET@` → `X86_WIN32` for x86 / `X86_WIN64` for x64,
  `@HAVE_LONG_DOUBLE@` → `0`, `@FFI_EXEC_TRAMPOLINE_TABLE@` → `0`).
  The script aborts if upstream introduces a new `@VAR@` token it
  doesn't know how to substitute, or if any token slips through
  unsubstituted. Both `x86/ffi.h` and `x64/ffi.h` are written on each
  run.

- **`fficonfig.h`** is autoheader output that depends on
  feature-detection results, so the script can't substitute it by
  hand. The committed copies are preserved across upgrades; if
  upstream's `fficonfig.h.in` template changes between versions the
  project-local `fficonfig.h` files may need to be regenerated by hand
  on a Linux/macOS host (the script prints the recipe at the end of
  its run):

  ```sh
  cd MagPython/libffi/libffi-X.Y.Z
  ./configure --host=i686-w64-mingw32 --enable-static
  cp fficonfig.h <repo>/MagPython/libffi-msvc-include/x86/fficonfig.h
  ./configure --host=x86_64-w64-mingw32 --enable-static
  cp fficonfig.h <repo>/MagPython/libffi-msvc-include/x64/fficonfig.h
  ```
